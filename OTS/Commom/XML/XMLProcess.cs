...
namespace Common.XML
{
    public class XMLProcess
    {
        ...
        private XmlDocument XMLLoad()
        {
            ...
            try
            {
                string filename = (AppDomain.CurrentDomain.BaseDirectory.ToString() + XMLFile).Replace("/","\\");
        ...
        private static string GetXmlFullPath(string strPath)
        {
            ...
            //{
            return AppDomain.CurrentDomain.BaseDirectory.ToString() + strPath;
            //}
            //else
            //{
        ...
        /// <param name="node">节点</param>
        /// <param name="attribute">属性名，非空时返回该属性值，否则返回串联值</param>
        /// 使用示列: //MessageBox.Show(XMLProcess.Read("Appconfig.xml", "Appconfig/File/attrib1", "StartDateTime"));
        /// XMLProsess.Read(path, "/Node", "")
        /// XMLProsess.Read(path, "/Node/Element[@Attribute='Name']", "Attribute")
        public string ReadAttribute(string node, string attribute)
        {
            string value = "";
            try
            {
        ...
        public static bool UpdateXmlRow(string strXmlPath, string[] Columns, string[] ColumnValue, string strWhereColumnName, string strWhereColumnValue)
        {
            try
            {
                string strXsdPath = strXmlPath.Substring(0, strXmlPath.IndexOf(".")) + ".xml";
                DataSet ds = new DataSet();
                ds.ReadXmlSchema(GetXmlFullPath(strXsdPath));//读XML架构，关系到列的数据类型
                ds.ReadXml(GetXmlFullPath(strXmlPath));
